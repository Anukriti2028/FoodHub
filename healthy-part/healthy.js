...
function readRestaurants(){
  return Array.from(restGrid.querySelectorAll(".rest")).map(el => ({
    ...
    tag: (el.dataset.tag || "").trim(),
    cuisines: el.dataset.cuisines || ""
  }));
}

function readMenuItems(rid){
  const card = restGrid.querySelector(`.rest[data-rid="${rid}"]`);
  if(!card) return [];
  const data = card.querySelector(".menu-data");
  if(!data) return [];
  return Array.from(data.querySelectorAll(".mi")).map(x => ({
    id: x.dataset.itemid,
    cat: x.dataset.cat,
    name: x.dataset.name,
    desc: x.dataset.desc,
...
function applyFilters(list){
  ...
  if(activeChip === "highprotein") out = out.filter(r => r.tag === "highprotein");
  if(activeChip === "keto") out = out.filter(r => r.tag === "keto");
